...
public class TileEntityBarrel extends TileEntityFactorization {
	...
	void info(EntityPlayer entityplayer) {
		if (item == null && getItemCount() == 0) {
			...
			mod_Factorization.instance.broadcastTranslate(entityplayer, "This barrel is full of %s", mod_Factorization.instance.translateItemStack(item));
		} else {
			mod_Factorization.instance.broadcastTranslate(entityplayer, "This barrel contains %s %s", "" + getItemCount(), mod_Factorization.instance.translateItemStack(item));
		}
	}
	...
	public void activate(EntityPlayer entityplayer) {
		...
		if (!itemMatch(is)) {
			if (mod_Factorization.instance.translateItemStack(is).equals(mod_Factorization.instance.translateItemStack(item))) {
	...
	void addAllItems(EntityPlayer entityplayer) {
		...
		if (hand != null && !itemMatch(hand)) {
			if (mod_Factorization.instance.translateItemStack(hand).equals(
					mod_Factorization.instance.translateItemStack(item))) {
				entityplayer.addChatMessage("That item is different");
			} else {
				info(entityplayer);
			}
