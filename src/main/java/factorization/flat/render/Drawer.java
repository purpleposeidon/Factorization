...
class Drawer implements IFlatVisitor {
    ...
    public void visit(Coord at, EnumFacing side, @Nonnull FlatFace face) {
        ...
        IBakedModel[] models = flatModel.getModel(at, side);
        if (models == null) return;
        for (IBakedModel model : models) {
            if (!normalObscured) {
                renderModel(model, at, side, color);
            }
            if (!oppositeObscured) {
                at.adjust(side);
                renderModel(model, at, opposite, color);
                at.adjust(opposite);
            }
        }
    }

    void renderModel(IBakedModel model, Coord at, EnumFacing side, int color) {
        if (!started) {
            tess.begin(GL11.GL_QUADS, DefaultVertexFormats.BLOCK);
            started = true;
        }
        modelRenderer.renderModelStandard(at.w, model, SideSelectors.get(side, color), at.toBlockPos(), tess, true);
    }
