...
public abstract class TileEntityFactorization extends TileEntityCommon
	...
	public void activate(EntityPlayer entityplayer) {
		...
		if (type.hasGui) {
			entityplayer.openGui(mod_Factorization.instance, type.gui, worldObj, xCoord, yCoord, zCoord);
	...
	public void drawActive(int add_time) {
		...
		if (draw_active != new_active) {
			...
			if (mod_Factorization.instance.isCannonical(worldObj)) {
	...
	void broadcastMessage(EntityPlayer who, int messageType, Object... msg) {
		mod_Factorization.network.broadcastMessage(who, getCoord(), messageType, msg);
	}
